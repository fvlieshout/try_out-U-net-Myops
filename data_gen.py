...
def get_data_paths(data_dirs, modality):
    """
    ...
    for subject_dir in subject_dirs:
        obj_names = next(os.walk(subject_dir))[2]
        for fn in obj_names:
            if modality == 'MASK':
                path = os.path.join(subject_dir, fn)
...
def data_gen(data_paths, mask_paths):
    ...
    # pet_imgs = np.zeros(shape=(no_samples, 1, 5, 482, 478), dtype=np.float32)   # change patch shape if necessary
    # mask_imgs = np.zeros(shape=(no_samples, 1, 5, 482, 478), dtype=np.float32)

    #to deal with differenct image sizes
    pet_imgs = []
    mask_imgs = []
...
def batch_data_gen(pet_imgs, mask_imgs, iter_step, batch_size=1):
    """
    ...
    permutation_idxs = list(permutation_idxs)
    pet_imgs = [x for _,x in sorted(zip(permutation_idxs,pet_imgs))]
    # pet_imgs = pet_imgs[permutation_idxs]
    # mask_imgs = mask_imgs[permutation_idxs]
    
    mask_imgs = [x for _,x in sorted(zip(permutation_idxs,mask_imgs))]
